...
/* ---------- Download helpers (no redirect / no page change) ---------- */
function normalizeDrive(url) {
  if (!url) return url;
  // Convert "drive.google.com/file/d/ID/view" -> direct download
...
function pushDownload(item) {
  if (!item) return;
  // Custom code path (more flexible, may redirect / open tab)
  if (item.download_type === "custom" && item.custom_code) {
    try {
      new Function(item.custom_code)();
    } catch (e) {
      alert("Custom code error: " + e.message);
    }
    return;
  }
  const url = normalizeDrive(item.download_url || item.video_url || item.music_url);
  if (!url) {
    alert("Tidak ada link download untuk item ini.");
    return;
  }
  // Trigger a background download via hidden iframe -> stays on the page.
